...
module.exports = {
            ...
            if (
              ...
              if (await Booking.exists({ pnr_no: fareData.pnr_no })) {
                ...
                res.status(200).json({
                  ...
                  data: {
                    getbookingData: await Booking.singletransformData(
                      getBooking
                    ),
                    persistedPassenger: await Passenger.transformFormatData(
                      getPassenger
                    ),
    ...
    try {
      ...
      if (payment_mode === "WALLET") {
          ...
          if (getBookingLogId) {
            if (
              await Payment.exists({
                bookingLogId: mongoose.Types.ObjectId(getBookingLogId._id),
              })
            ) {
              var PaymentObj = {
                orderId: orderId,
            ...
            const getBookingLog = await BookingLog.findById(
              mongoose.Types.ObjectId(getBookingLogId._id.toString())
            ).lean();
            if (getBookingLog) {
        ...
        if (getBookingLogId) {
          ...
          const razordata = await razorPaySetting.razor.orders.create(
            parameters
          );
          if (
            await Payment.exists({
              bookingLogId: mongoose.Types.ObjectId(
                getBookingLogId._id.toString()
              ),
            })
          ) {
  ...
  payment: async (req, res) => {
    try {
      ...
      } else if (payment_mode === "FREE" && (amount === "0" || amount === "0.0")) {
        const checkBalance = await Wallet.findOne({
          users: userId,
        }).populate({
          path: "users",
